...
--pedido sem itens 
-- interno altera e retorna o modificado 
-- sigla p é para parametro 
    --agr preciso ter o cod desse pedido vazio 
--o lastval pega esse valor e gurda em uma variavel 
CREATe OR REPLACE PROCEDURE sp_criar_pedido_sem_itens 
...
BEGIN
 SELECT c.cod_cliente FROM tb_cliente c 
    WHERE nome LIKE 'Ana Silva' INTO v_code_cliente;
 CAll sp_criar_pedido_sem_itens ( v_code_pedido, v_code_cliente);
 Raise NOTICE 'Código do pedido recém criado: %', v_cod_pedido;
END;
$$

CREATE OR REPLACE PROCEDURE sp_adicionar_item_a_pedido
(IN p_cod_pedido INT, IN p_cod_item INT)
LANGUAGE plpgsql
AS $$
BEGIN
    INSERT INTO tb_item_pedido (cod_item, cod_pedido) VALUES (p_cod_pedido, p_cod_item);
